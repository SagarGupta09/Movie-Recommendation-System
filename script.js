...
document.addEventListener("DOMContentLoaded", function() {
    ...
    toggleButton.addEventListener("click", function() {
        // Toggle the 'hide' class of the box
        boxBorder.classList.toggle("hide");
    });
});

document.addEventListener("DOMContentLoaded", function() {
    // Get the button
    var recommendButton = document.getElementById("recButton");
...
async function recommendMovies() {
    console.log("clicked")
    const genre = document.querySelector('input[name="genre"]:checked').value; // Get the selected genre
    const url = `https://www.omdbapi.com/?s=&page=1&apikey=${key}&type=movie&s=${genre}`; // Include the genre parameter in the URL
    const res = await fetch(url);
    const data = await res.json();
